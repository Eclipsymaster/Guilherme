...
if NpcHandler == nil then
	...
	NpcHandler = {
		...
		messages = {
			...
			[MESSAGE_GREET] = "Opa, |PLAYERNAME|.",
			[MESSAGE_FAREWELL] = "ate mais, |PLAYERNAME|.",
			[MESSAGE_BUY] = "Voce quer comprar |ITEMCOUNT| |ITEMNAME| por |TOTALCOST| moedas de ouro?",
			[MESSAGE_MISSINGMONEY] = "Voce nao tem dinheiro.",
			[MESSAGE_NEEDMONEY] = "Voce nao tem dinheiro.",
			[MESSAGE_MISSINGITEM] = "Voce nao tem a quantia.",
			[MESSAGE_NEEDITEM] = "Voce nao tem esse objeto.",
			[MESSAGE_NEEDSPACE] = "Nao ha espaco sufuciente em sua bolsa",
			[MESSAGE_NEEDMORESPACE] = "Voce nao tem capacidade para todos os itens.",
			[MESSAGE_WALKAWAY] = "Vai com Deus.",
			[MESSAGE_DECLINE] = "Entao nao.",
			[MESSAGE_SENDTRADE] = "Claro, de uma olhada nos meus produtos",
			[MESSAGE_NOSHOP] = "Desculpe nao estou vendendo nada.",
			[MESSAGE_ONCLOSESHOP] = "Obrigado, volte sempre se precisar de mais alguma coisa",
			[MESSAGE_ALREADYFOCUSED] = "Eu ja estou falando com voce |PLAYERNAME|",
	...
	function NpcHandler:onWalkAway(npc, player)
		...
		if callback == nil or callback() then
			if self:processModuleCallback(CALLBACK_ON_DISAPPEAR, npc, player) then
				...
				local playerSex = player and player:getPlayerRace() or 0
