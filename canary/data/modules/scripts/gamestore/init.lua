...
function parseBuyStoreOffer(playerId, msg)
	...
	local pcallOk, pcallError = pcall(function()
		...
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_OUTFIT then
			GameStore.processOutfitPurchase(player, offer.racePlayerId, offer.addon)
...
function Player.canBuyOffer(self, offer)
	...
	if disabled ~= 1 then
		...
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_OUTFIT or offer.type == GameStore.OfferTypes.OFFER_TYPE_OUTFIT_ADDON then
			...
			if self:getPlayerRace() == HUMAN then
				outfitLookType = offer.racePlayerId.humanRace
			elseif self:getPlayerRace() == ORC then
				outfitLookType = offer.racePlayerId.orcRace
			elseif self:getPlayerRace() == ELF then
				outfitLookType = offer.racePlayerId.elfRace
			elseif self:getPlayerRace() == MORTOVIVO then
				outfitLookType = offer.racePlayerId.mortovivoRace
			elseif self:getPlayerRace() == ANAO then
				outfitLookType = offer.racePlayerId.anaoRace
			end
...
function sendShowStoreOffers(playerId, category, redirectId)
	...
	for k, offer in ipairs(category.offers) do
		...
		if not offers[name] then
			...
			if offer.racePlayerId then
				offers[name].racePlayerId = offer.racePlayerId
			end
	...
	for name, offer in pairs(offers) do
		if count > 0 then
			...
			elseif type == GameStore.ConverType.SHOW_OUTFIT then
				msg:addU16( (player:getPlayerRace() == HUMAN and offer.racePlayerId.humanRace) or 
							(player:getPlayerRace() == ORC and offer.racePlayerId.orcRace) or 
							(player:getPlayerRace() == ELF and offer.racePlayerId.elfRace) or
							(player:getPlayerRace() == MORTOVIVO and offer.racePlayerId.mortovivoRace) or
							(player:getPlayerRace() == ANAO and offer.racePlayerId.anaoRace))
			...
			elseif type == GameStore.ConverType.SHOW_HIRELING then
				if player:getPlayerRace() == ORC then
					msg:addByte(1)
				else
					msg:addByte(2)
				end
				msg:addU16(offer.racePlayerId.humanRace)
				msg:addU16(offer.racePlayerId.orcRace)
				msg:addU16(offer.racePlayerId.elfRace)
				msg:addU16(offer.racePlayerId.mortovivoRace)
				msg:addU16(offer.racePlayerId.anaoRace)
				local outfit = player:getOutfit()
				msg:addByte(outfit.lookHead)
				msg:addByte(outfit.lookBody)
				msg:addByte(outfit.lookLegs)
...
function GameStore.processOutfitPurchase(player, offerRaceTable, addon)
	local looktype
	local _addon = addon and addon or 0

	if player:getPlayerRace() == ORC then
		looktype = offerRaceTable.orcRace
	elseif player:getPlayerRace() == HUMAN then
		looktype = offerRaceTable.humanRace
	elseif player:getPlayerRace() == ELF then
		looktype = offerRaceTable.elfRace
	elseif player:getPlayerRace() == MORTOVIVO then
		looktype = offerRaceTable.mortovivoRace
	elseif player:getPlayerRace() == ANAO then
		looktype = offerRaceTable.anaoRace
	end
	...
	elseif player:hasOutfit(looktype, _addon) then
		...
		then
			...
			player:addOutfitAddon(offerRaceTable.humanRace, _addon)
			player:addOutfitAddon(offerRaceTable.orcRace, _addon)
			player:addOutfitAddon(offerRaceTable.elfRace, _addon)
			player:addOutfitAddon(offerRaceTable.mortovivoRace, _addon)
			player:addOutfitAddon(offerRaceTable.anaoRace, _addon)
		end
	end
end
...
function Player.toggleSex(self)
	local currentSex = self:getPlayerRace()
	local playerOutfit = self:getOutfit()

	playerOutfit.lookAddons = 0
	if currentSex == HUMAN then
		self:setPlayerRace(ORC)
		playerOutfit.lookType = 128
	else
		self:setPlayerRace(HUMAN)
