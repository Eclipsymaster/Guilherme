...
function iceDeath.onStepIn(creature, item, position, fromPosition)
	...
	if player:getStorageValue(Storage.CultsOfTibia.Barkless.Ice) == 2 then
		...
		condition:setOutfit(player:getPlayerRace() == HUMAN and 4247 or 4240)
		player:addCondition(condition)
		local it = Game.createItem(player:getPlayerRace() == HUMAN and 4247 or 4240, 1, player:getPosition())
		if it then
			it:decay()
		end
