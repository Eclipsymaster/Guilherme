...
local function creatureSayCallback(npc, creature, type, message)
	...
	if MsgContains(message, "missions") then
		...
		elseif player:getStorageValue(Storage.CultsOfTibia.FinalBoss.Mission) == 2 then
			...
			local item = ""
			if player:getVocation():getBaseId() == VOCATION.BASE_ID.HUMANO_CLERIGO then
				player:addItem(26190)
				item = "reflecting crown"
			end
			if player:getVocation():getBaseId() == VOCATION.BASE_ID.HUMANO_PALADINO then
				player:addItem(26187)
				item = "leaf crown"
			end
			if player:getVocation():getBaseId() == VOCATION.BASE_ID.HUMANO_MAGO then
				player:addItem(26189)
				item = "incandescent crown"
			end
			if player:getVocation():getBaseId() == VOCATION.BASE_ID.HUMANO_VERETANO then
