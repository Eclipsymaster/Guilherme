...
local hatKeyword = keywordHandler:addKeyword({ "proof" }, StdModule.say, { npcHandler = npcHandler, text = "... I cannot believe my eyes. You retrieved this hat from Ferumbras' remains? That is incredible. If you give it to me, I will grant you the right to wear this hat as addon. What do you say?" }, function(player)
	return not player:hasOutfit(player:getPlayerRace() == HUMAN and 141 or 130, 2)
end)
hatKeyword:addChildKeyword({ "yes" }, StdModule.say, { npcHandler = npcHandler, text = "Sorry you don't have the Ferumbras' hat." }, function(player)
	return player:getItemCount(5903) == 0
end)
hatKeyword:addChildKeyword({ "yes" }, StdModule.say, { npcHandler = npcHandler, text = "I bow to you, player, and hereby grant you the right to wear Ferumbras' hat as accessory. Congratulations!" }, nil, function(player)
...
keywordHandler:addKeyword({ "myra" }, StdModule.say, {
	...
	return player:getStorageValue(Storage.OutfitQuest.MageSummoner.AddonHatCloak) == 10
end, function(player)
...
keywordHandler:addKeyword({ "myra" }, StdModule.say, { npcHandler = npcHandler, text = "Stop bothering me. I am a far too busy man to be constantly giving out awards." }, function(player)
	return player:getStorageValue(Storage.OutfitQuest.MageSummoner.AddonHatCloak) == 11
end)
