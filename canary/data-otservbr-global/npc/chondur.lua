...
local function notReadyKeyword(keyword, text)
	keywordHandler:addKeyword({ keyword }, StdModule.say, { npcHandler = npcHandler, text = text }, function(player)
		return not player:hasOutfit(player:getPlayerRace() == HUMAN and 158 or 154)
	end)
end

notReadyKeyword("outfit", {
...
local function addTaskKeyword(text, value, missionStorage)
	local taskKeyword = keywordHandler:addKeyword({ "task" }, StdModule.say, { npcHandler = npcHandler, text = text[1] }, function(player)
	...
	yesKeyword:addChildKeyword({ "yes" }, StdModule.say, { npcHandler = npcHandler, text = text[3], reset = true }, nil, function(player)
...
local function addItemKeyword(keyword, aliasKeyword, text, value, item, addonId, missionStorage, achievement)
	local itemKeyword = keywordHandler:addKeyword({ keyword }, StdModule.say, { npcHandler = npcHandler, text = text[1] }, function(player)
		return player:getStorageValue(Storage.OutfitQuest.Shaman.AddonStaffMask) == value
	end)
	itemKeyword:addChildKeyword({ "yes" }, StdModule.say, { npcHandler = npcHandler, text = text[2], reset = true }, function(player)
		return player:getItemCount(item[1].itemId) < item[1].count or player:getItemCount(item[2].itemId) < item[2].count
	end)

	itemKeyword:addChildKeyword({ "yes" }, StdModule.say, { npcHandler = npcHandler, text = text[3], reset = true }, function(player)
...
}, 1, { { itemId = 5014, count = 1 }, { itemId = 3002, count = 5 } }, 2, Storage.OutfitQuest.Shaman.MissionStaff)
...
local stakeKeyword = keywordHandler:addKeyword({ "stake" }, StdModule.say, { npcHandler = npcHandler, text = "Would you like to receive a spiritual prayer to bless your stake?" }, function(player)
	return player:getStorageValue(Storage.FriendsandTraders.TheBlessedStake) == 12
end)

stakeKeyword:addChildKeyword({ "yes" }, StdModule.say, { npcHandler = npcHandler, text = "You don't have a wooden stake.", reset = true }, function(player)
...
stakeKeyword:addChildKeyword({ "yes" }, StdModule.say, { npcHandler = npcHandler, text = "<mumblemumble> Sha Kesh Mar!", reset = true }, function(player)
	return player:getItemCount(5941) > 0
end, function(player)
...
-- Counterspell
keywordHandler:addKeyword({ "counterspell" }, StdModule.say, { npcHandler = npcHandler, text = "You should not talk about things you don't know anything about." }, function(player)
	return player:getStorageValue(Storage.TheShatteredIsles.DragahsSpellbook) == -1
end)
keywordHandler:addAliasKeyword({ "energy field" })

-- Start mission
local startcounterspellKeyword = keywordHandler:addKeyword({ "counterspell" }, StdModule.say, {
	npcHandler = npcHandler,
...
local acceptKeyword = startcounterspellKeyword:addChildKeyword({ "yes" }, StdModule.say, {
	npcHandler = npcHandler,
...
startcounterspellKeyword:addChildKeyword({ "no" }, StdModule.say, { npcHandler = npcHandler, text = "It's much safer for you to stay here anyway, trust me.", reset = true })

-- Deliver in corpses
local function addCounterspellKeyword(text, value, itemId)
	local counterspellKeyword = keywordHandler:addKeyword({ "counterspell" }, StdModule.say, { npcHandler = npcHandler, text = text[1] }, function(player)
		return player:getStorageValue(Storage.TheShatteredIsles.TheCounterspell) == value
	end)
	counterspellKeyword:addChildKeyword({ "yes" }, StdModule.say, { npcHandler = npcHandler, text = text[2], reset = true }, function(player)
...
keywordHandler:addKeyword({ "counterspell" }, StdModule.say, {
	npcHandler = npcHandler,
	text = "Hm. I don't think you need another one of my counterspells to cross the barrier on Goroma.",
})

-- Spellbook
keywordHandler:addKeyword({ "spellbook" }, StdModule.say, { npcHandler = npcHandler, text = "Ah, thank you very much! I'll honour his memory." }, function(player)
...
-- Energy Field
keywordHandler:addKeyword({ "energy field" }, StdModule.say, {
	npcHandler = npcHandler,
