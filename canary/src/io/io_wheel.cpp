...
bool IOWheel::initializeGlobalData(bool reload /* = false*/) {
	...
	// Register spells for humanos
	for (const auto &data : getWheelBonusData().spells.Humano) {
	...
	// Register spells for orcs
	for (const auto &data : getWheelBonusData().spells.Orc) {
		for (size_t i = 1; i < 3; ++i) {
			const auto &grade = data.grade[i];
			InternalPlayerWheel::registerWheelSpellTable(grade, data.name, static_cast<WheelSpellGrade_t>(i));
		}
	...
	// Register spells for elfs
	for (const auto &data : getWheelBonusData().spells.Elf) {
		for (size_t i = 1; i < 3; ++i) {
			const auto &grade = data.grade[i];
			InternalPlayerWheel::registerWheelSpellTable(grade, data.name, static_cast<WheelSpellGrade_t>(i));
		}
	...
	// Register spells for Mortos vivos
	for (const auto &data : getWheelBonusData().spells.Mortovivo) {
		for (size_t i = 1; i < 3; ++i) {
			const auto &grade = data.grade[i];
			InternalPlayerWheel::registerWheelSpellTable(grade, data.name, static_cast<WheelSpellGrade_t>(i));
		}
	...
	// Register spells for anoes
	for (const auto &data : getWheelBonusData().spells.Anao) {
		for (size_t i = 1; i < 3; ++i) {
			const auto &grade = data.grade[i];
			InternalPlayerWheel::registerWheelSpellTable(grade, data.name, static_cast<WheelSpellGrade_t>(i));
		}
...
void IOWheel::initializeMapData() {
	...
	initializeHumanSpells();
	initializeOrcSpells();
	initializeElfSpells();
	initializeMortovivoSpells();
	initializeAnaoSpells();
}

void IOWheel::initializeHumanSpells() {
	m_wheelBonusData.spells.Humano[0].name = "Strong Ice Wave";
	m_wheelBonusData.spells.Humano[0].grade[1].leech.mana = 3;
	m_wheelBonusData.spells.Humano[0].grade[2].increase.damage = 30;

	m_wheelBonusData.spells.Humano[1].name = "Mass Healing";
	m_wheelBonusData.spells.Humano[1].grade[1].increase.heal = 10;
	m_wheelBonusData.spells.Humano[1].grade[2].increase.area = true;

	m_wheelBonusData.spells.Humano[2].name = "Nature's Embrace";
	m_wheelBonusData.spells.Humano[2].grade[1].increase.heal = 10;
	m_wheelBonusData.spells.Humano[2].grade[2].decrease.cooldown = 10;

	m_wheelBonusData.spells.Humano[3].name = "Terra Wave";
	m_wheelBonusData.spells.Humano[3].grade[1].increase.damage = 25;
	m_wheelBonusData.spells.Humano[3].grade[2].leech.life = 5;

	m_wheelBonusData.spells.Humano[4].name = "Heal Friend";
	m_wheelBonusData.spells.Humano[4].grade[1].decrease.manaCost = 10;
	m_wheelBonusData.spells.Humano[4].grade[2].increase.heal = 10;
}

void IOWheel::initializeOrcSpells() {
	m_wheelBonusData.spells.Orc[0].name = "Front Sweep";
	m_wheelBonusData.spells.Orc[0].grade[1].leech.life = 5;
	m_wheelBonusData.spells.Orc[0].grade[2].increase.damage = 30;

	m_wheelBonusData.spells.Orc[1].name = "Groundshaker";
	m_wheelBonusData.spells.Orc[1].grade[1].increase.damage = 25;
	m_wheelBonusData.spells.Orc[1].grade[2].decrease.cooldown = 2;

	m_wheelBonusData.spells.Orc[2].name = "Chivalrous Challenge";
	m_wheelBonusData.spells.Orc[2].grade[1].decrease.manaCost = 20;
	m_wheelBonusData.spells.Orc[2].grade[2].increase.aditionalTarget = 1;

	m_wheelBonusData.spells.Orc[3].name = "Intense Wound Cleansing";
	m_wheelBonusData.spells.Orc[3].grade[1].increase.heal = 10;
	m_wheelBonusData.spells.Orc[3].grade[2].decrease.cooldown = 300;

	m_wheelBonusData.spells.Orc[4].name = "Fierce Berserk";
	m_wheelBonusData.spells.Orc[4].grade[1].decrease.manaCost = 30;
	m_wheelBonusData.spells.Orc[4].grade[2].increase.damage = 25;
}

void IOWheel::initializeElfSpells() {
	m_wheelBonusData.spells.Elf[0].name = "Sharpshooter";
	m_wheelBonusData.spells.Elf[0].grade[1].decrease.secondaryGroupCooldown = 8;
	m_wheelBonusData.spells.Elf[0].grade[2].decrease.cooldown = 6;

	m_wheelBonusData.spells.Elf[1].name = "Strong Ethereal Spear";
	m_wheelBonusData.spells.Elf[1].grade[1].decrease.cooldown = 2;
	m_wheelBonusData.spells.Elf[1].grade[2].increase.damage = 25;

	m_wheelBonusData.spells.Elf[2].name = "Divine Dazzle";
	m_wheelBonusData.spells.Elf[2].grade[1].increase.aditionalTarget = 1;
	m_wheelBonusData.spells.Elf[2].grade[2].increase.duration = 4;
	m_wheelBonusData.spells.Elf[2].grade[2].decrease.cooldown = 4;

	m_wheelBonusData.spells.Elf[3].name = "Swift Foot";
	m_wheelBonusData.spells.Elf[3].grade[1].decrease.secondaryGroupCooldown = 8;
	m_wheelBonusData.spells.Elf[3].grade[2].decrease.cooldown = 6;

	m_wheelBonusData.spells.Elf[4].name = "Divine Caldera";
	m_wheelBonusData.spells.Elf[4].grade[1].decrease.manaCost = 20;
	m_wheelBonusData.spells.Elf[4].grade[2].increase.damage = 25;
}

void IOWheel::initializeMortovivoSpells() {
	m_wheelBonusData.spells.Mortovivo[0].name = "Magic Shield";
	m_wheelBonusData.spells.Mortovivo[0].grade[2].decrease.cooldown = 6;

	m_wheelBonusData.spells.Mortovivo[1].name = "Sap Strength";
	m_wheelBonusData.spells.Mortovivo[1].grade[1].increase.area = true;
	m_wheelBonusData.spells.Mortovivo[1].grade[2].increase.damageReduction = 10;

	m_wheelBonusData.spells.Mortovivo[2].name = "Energy Wave";
	m_wheelBonusData.spells.Mortovivo[2].grade[1].increase.damage = 25;
	m_wheelBonusData.spells.Mortovivo[2].grade[2].increase.area = true;

	m_wheelBonusData.spells.Mortovivo[3].name = "Great Fire Wave";
	m_wheelBonusData.spells.Mortovivo[3].grade[1].increase.criticalDamage = 15;
	m_wheelBonusData.spells.Mortovivo[3].grade[1].increase.criticalChance = 10;
	m_wheelBonusData.spells.Mortovivo[3].grade[2].increase.damage = 25;

	m_wheelBonusData.spells.Mortovivo[4].name = "Any_Focus_Mage_Spell";
	m_wheelBonusData.spells.Mortovivo[4].grade[1].increase.damage = 25;
	m_wheelBonusData.spells.Mortovivo[4].grade[2].decrease.cooldown = 4;
	m_wheelBonusData.spells.Mortovivo[4].grade[2].decrease.secondaryGroupCooldown = 4;
}

void IOWheel::initializeAnaoSpells() {
	m_wheelBonusData.spells.Anao[0].name = "Magic Shield";
	m_wheelBonusData.spells.Anao[0].grade[2].decrease.cooldown = 6;

	m_wheelBonusData.spells.Anao[1].name = "Sap Strength";
	m_wheelBonusData.spells.Anao[1].grade[1].increase.area = true;
	m_wheelBonusData.spells.Anao[1].grade[2].increase.damageReduction = 10;

	m_wheelBonusData.spells.Anao[2].name = "Energy Wave";
	m_wheelBonusData.spells.Anao[2].grade[1].increase.damage = 25;
	m_wheelBonusData.spells.Anao[2].grade[2].increase.area = true;

	m_wheelBonusData.spells.Anao[3].name = "Great Fire Wave";
	m_wheelBonusData.spells.Anao[3].grade[1].increase.criticalDamage = 15;
	m_wheelBonusData.spells.Anao[3].grade[1].increase.criticalChance = 10;
	m_wheelBonusData.spells.Anao[3].grade[2].increase.damage = 25;

	m_wheelBonusData.spells.Anao[4].name = "Any_Focus_Mage_Spell";
	m_wheelBonusData.spells.Anao[4].grade[1].increase.damage = 25;
	m_wheelBonusData.spells.Anao[4].grade[2].decrease.cooldown = 4;
	m_wheelBonusData.spells.Anao[4].grade[2].decrease.secondaryGroupCooldown = 4;
}
...
bool IOWheel::isHumano(uint8_t vocationId) const {
	return vocationId == Vocation_t::VOCATION_HUMANO_CIP;
}

bool IOWheel::isOrc(uint8_t vocationId) const {
	return vocationId == Vocation_t::VOCATION_ORC_CIP;
}

bool IOWheel::isElf(uint8_t vocationId) const {
	return vocationId == Vocation_t::VOCATION_ELF_CIP;
}

bool IOWheel::isMortovivo(uint8_t vocationId) const {
	return vocationId == Vocation_t::VOCATION_MORTOVIVO_CIP;
}

bool IOWheel::isAnao(uint8_t vocationId) const {
	return vocationId == Vocation_t::VOCATION_ANAO_CIP;
}
...
void IOWheel::slotGreen200(const std::shared_ptr<Player> &player, uint16_t points, uint8_t vocationCipId, PlayerWheelMethodsBonusData &bonusData) const {
	...
	if (isHumano(vocationCipId)) {
		...
		if (pointsInSlot && isElf(vocationCipId)) {
			bonusData.instant.runicMastery = true;
		} else if (pointsInSlot && isMortovivo(vocationCipId)) {
...
void IOWheel::slotBlueBottom100(const std::shared_ptr<Player> &player, uint16_t points, uint8_t vocationCipId, PlayerWheelMethodsBonusData &bonusData) const {
	...
	if (isHumano(vocationCipId) && onSlot) {
		bonusData.skills.melee += 1;
	} else if (isOrc(vocationCipId) && onSlot) {
		bonusData.skills.distance += 1;
	} else if (onSlot && (isElf(vocationCipId) || isMortovivo(vocationCipId))) {
		bonusData.skills.magic += 1;
	}
}
