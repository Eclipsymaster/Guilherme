...
void ItemParse::parseBeds(const std::string &tmpStrValue, pugi::xml_attribute valueAttribute, ItemType &itemType) {
	...
	if (stringValue == "maletransformto") {
		...
		itemType.transformToOnUse[ORC] = valueMale;
		if (other.transformToFree == 0) {
			other.transformToFree = itemType.id;
		}

		if (itemType.transformToOnUse[HUMAN] == 0) {
			itemType.transformToOnUse[HUMAN] = valueMale;
		}
		...
		itemType.transformToOnUse[HUMAN] = valueFemale;

		if (other.transformToFree == 0) {
			other.transformToFree = itemType.id;
		}

		if (itemType.transformToOnUse[ORC] == 0) {
			itemType.transformToOnUse[ORC] = valueFemale;
		}
	}
