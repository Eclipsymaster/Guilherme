...
void BedItem::updateAppearance(std::shared_ptr<Player> player) {
	...
	if (it.type == ITEM_TYPE_BED) {
		if ((player != nullptr) && it.transformToOnUse[player->getPlayerRace()] != 0) {
			const ItemType &newType = Item::items[it.transformToOnUse[player->getPlayerRace()]];
			if (newType.type == ITEM_TYPE_BED) {
				g_game().transformItem(static_self_cast<BedItem>(), it.transformToOnUse[player->getPlayerRace()]);
			}
