...
void ProtocolGame::sendCyclopediaCharacterOutfitsMounts() {
	...
	const auto outfits = Outfits::getInstance().getOutfits(player->getPlayerRace());
...
void ProtocolGame::sendCyclopediaCharacterInspection() {
	...
	if (const auto outfit = Outfits::getInstance().getOutfitByLookType(player->getPlayerRace(), player->getDefaultOutfit().lookType)) {
