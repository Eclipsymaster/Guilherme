...
public:
	...
	std::shared_ptr<Outfit> getOpositeSexOutfitByLookType(PlayerRace_t sex, uint16_t lookType);

	bool loadFromXml();

	[[nodiscard]] std::shared_ptr<Outfit> getOutfitByLookType(PlayerRace_t sex, uint16_t lookType) const;
	[[nodiscard]] const std::vector<std::shared_ptr<Outfit>> &getOutfits(PlayerRace_t sex) const {
		return outfits[sex];
	}
