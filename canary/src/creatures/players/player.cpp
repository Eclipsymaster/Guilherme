...
void Player::drainHealth(std::shared_ptr<Creature> attacker, int32_t damage) {
	if (PLAYER_SOUND_HEALTH_CHANGE >= static_cast<uint32_t>(uniform_random(1, 100))) {
		g_game().sendSingleSoundEffect(static_self_cast<Player>()->getPosition(), sex == HUMAN ? SoundEffect_t::HUMAN_FEMALE_BARK : SoundEffect_t::HUMAN_MALE_BARK, getPlayer());
	}
...
void Player::death(std::shared_ptr<Creature> lastHitCreature) {
	...
	g_game().sendSingleSoundEffect(static_self_cast<Player>()->getPosition(), sex == HUMAN ? SoundEffect_t::HUMAN_FEMALE_DEATH : SoundEffect_t::HUMAN_MALE_DEATH, getPlayer());
	if (skillLoss) {
...
void Player::setPlayerRace(PlayerRace_t newSex) {
	sex = newSex;
}
...
bool Player::toggleMount(bool mount) {
	...
	if (mount) {
		...
		const auto &playerOutfit = Outfits::getInstance().getOutfitByLookType(getPlayerRace(), defaultOutfit.lookType);
		if (!playerOutfit) {
			return false;
		}
