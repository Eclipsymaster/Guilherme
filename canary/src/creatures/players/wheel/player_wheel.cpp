...
bool PlayerWheel::getSpellAdditionalArea(const std::string &spellName) const {
	...
	if (vocationEnum == Vocation_t::VOCATION_HUMANO_CIP) {
		return checkSpellArea(g_game().getIOWheel()->getWheelBonusData().spells.Humano, spellName, stage);
	} else if (vocationEnum == Vocation_t::VOCATION_ORC_CIP) {
		return checkSpellArea(g_game().getIOWheel()->getWheelBonusData().spells.Orc, spellName, stage);
	} else if (vocationEnum == Vocation_t::VOCATION_ELF_CIP) {
		return checkSpellArea(g_game().getIOWheel()->getWheelBonusData().spells.Elf, spellName, stage);
	} else if (vocationEnum == Vocation_t::VOCATION_MORTOVIVO_CIP) {
		return checkSpellArea(g_game().getIOWheel()->getWheelBonusData().spells.Mortovivo, spellName, stage);
	} else if (vocationEnum == Vocation_t::VOCATION_ANAO_CIP) {
		return checkSpellArea(g_game().getIOWheel()->getWheelBonusData().spells.Anao, spellName, stage);
	}
...
int PlayerWheel::getSpellAdditionalTarget(const std::string &spellName) const {
	...
	auto vocationEnum = getPlayerVocationEnum();
	if (vocationEnum == Vocation_t::VOCATION_HUMANO_CIP) {
		return checkSpellAdditionalTarget(g_game().getIOWheel()->getWheelBonusData().spells.Humano, spellName, stage);
	} else if (vocationEnum == Vocation_t::VOCATION_ORC_CIP) {
		return checkSpellAdditionalTarget(g_game().getIOWheel()->getWheelBonusData().spells.Orc, spellName, stage);
	} else if (vocationEnum == Vocation_t::VOCATION_ELF_CIP) {
		return checkSpellAdditionalTarget(g_game().getIOWheel()->getWheelBonusData().spells.Elf, spellName, stage);
	} else if (vocationEnum == Vocation_t::VOCATION_MORTOVIVO_CIP) {
		return checkSpellAdditionalTarget(g_game().getIOWheel()->getWheelBonusData().spells.Mortovivo, spellName, stage);
	} else if (vocationEnum == Vocation_t::VOCATION_ANAO_CIP) {
		return checkSpellAdditionalTarget(g_game().getIOWheel()->getWheelBonusData().spells.Anao, spellName, stage);
	}
...
int PlayerWheel::getSpellAdditionalDuration(const std::string &spellName) const {
	...
	auto vocationEnum = getPlayerVocationEnum();
	if (vocationEnum == Vocation_t::VOCATION_HUMANO_CIP) {
		return checkSpellAdditionalDuration(g_game().getIOWheel()->getWheelBonusData().spells.Humano, spellName, stage);
	} else if (vocationEnum == Vocation_t::VOCATION_ORC_CIP) {
		return checkSpellAdditionalDuration(g_game().getIOWheel()->getWheelBonusData().spells.Orc, spellName, stage);
	} else if (vocationEnum == Vocation_t::VOCATION_ELF_CIP) {
		return checkSpellAdditionalDuration(g_game().getIOWheel()->getWheelBonusData().spells.Elf, spellName, stage);
	} else if (vocationEnum == Vocation_t::VOCATION_MORTOVIVO_CIP) {
		return checkSpellAdditionalDuration(g_game().getIOWheel()->getWheelBonusData().spells.Mortovivo, spellName, stage);
	} else if (vocationEnum == Vocation_t::VOCATION_ANAO_CIP) {
		return checkSpellAdditionalDuration(g_game().getIOWheel()->getWheelBonusData().spells.Anao, spellName, stage);
	}
...
uint8_t PlayerWheel::getPlayerVocationEnum() const {
	...
	if (cipTibiaId == 1 || cipTibiaId == 6 || cipTibiaId == 7 || cipTibiaId == 8 || cipTibiaId == 9) {
		return Vocation_t::VOCATION_HUMANO_CIP; // Humanos
	} else if (cipTibiaId == 2 || cipTibiaId == 10 || cipTibiaId == 11 || cipTibiaId == 12 || cipTibiaId == 13) {
		return Vocation_t::VOCATION_ORC_CIP; // Orcs
	} else if (cipTibiaId == 3 || cipTibiaId == 14 || cipTibiaId == 15 || cipTibiaId == 16 || cipTibiaId == 17) {
		return Vocation_t::VOCATION_ELF_CIP; // Elfs
	} else if (cipTibiaId == 4 || cipTibiaId == 18 || cipTibiaId == 19 || cipTibiaId == 20 || cipTibiaId == 21) {
		return Vocation_t::VOCATION_MORTOVIVO_CIP; // Mortos vivos
	} else if (cipTibiaId == 5 || cipTibiaId == 22 || cipTibiaId == 23 || cipTibiaId == 24 || cipTibiaId == 25) {
		return Vocation_t::VOCATION_ANAO_CIP; // Anões
	}
...
void PlayerWheel::loadDedicationAndConvictionPerks() {
	...
	if (vocationCipId < VOCATION_HUMANO_CIP || vocationCipId > VOCATION_MORTOVIVO_CIP) {
		return;
	}
