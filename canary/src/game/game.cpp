...
void Game::playerSetShowOffSocket(uint32_t playerId, Outfit_t &outfit, const Position &pos, uint8_t stackPos, const uint16_t itemId, uint8_t podiumVisible, uint8_t direction) {
	...
	if (outfit.lookType != 0 || outfit.lookMount != 0) {
		...
		bool outfited = false;
		if (outfit.lookType != 0) {
			const auto &outfitInfo = Outfits::getInstance().getOutfitByLookType(player->getPlayerRace(), outfit.lookType);
			if (!outfitInfo) {
				return;
			}
...
void Game::playerLookInShop(uint32_t playerId, uint16_t itemId, uint8_t count) {
	...
	std::ostringstream ss;
	ss << "Voce ve " << Item::getDescription(it, 1, nullptr, count);
...
void Game::playerChangeOutfit(uint32_t playerId, Outfit_t outfit, uint8_t isMountRandomized /* = 0*/) {
	...
	const auto playerOutfit = Outfits::getInstance().getOutfitByLookType(player->getPlayerRace(), outfit.lookType);
	if (!playerOutfit) {
		outfit.lookMount = 0;
	}
...
void Game::playerNpcGreet(uint32_t playerId, uint32_t npcId) {
	...
	internalCreatureSay(player, TALKTYPE_SAY, "oi", false, &spectators);

	auto npcsSpectators = spectators.filter<Npc>();

	if (npc->getSpeechBubble() == SPEECHBUBBLE_TRADE) {
		internalCreatureSay(player, TALKTYPE_PRIVATE_PN, "troca", false, &npcsSpectators);
	} else {
		internalCreatureSay(player, TALKTYPE_PRIVATE_PN, "velejar", false, &npcsSpectators);
	}
}
